...
class Phpfox_Installer
{
	...
	private function _key()
	{
		...
		if ($aVals = $this->_oReq->getArray('val'))
		{
			if ($oValid->isValid($aVals))
			{
				...
				if (($site_details = $oApi->send('auth', array('token' => $aVals['token']))))
				{
					file_put_contents(PHPFOX_DIR_FILE . 'setting' . PHPFOX_DS . 'license.php', "<?php define('MOXI9_PUBLIC_KEY', '" . $site_details['public_key'] . "'); define('MOXI9_PRIVATE_KEY', '" . $site_details['private_key'] . "'); ?>");
	...
	private function _theme() {
		...
		$theme = file_get_contents('http://d3u7rnto8gwhr0.cloudfront.net/unity/' . Phpfox::VERSION . '/' . Phpfox::PRODUCT_BUILD . '/theme.json');
		if (substr($theme, 0, 1) != '{') {
