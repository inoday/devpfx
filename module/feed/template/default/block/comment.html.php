...
<div id="js_feed_like_holder_{$aFeed.type_id}_{$aFeed.item_id}" class="comment_mini_content_holder">
        <div class="comment_mini_content_holder_icon"></div>
			<div class="comment_mini_content_border">
				...
				<div class="activity_action_bar {if Phpfox::isUser() && Phpfox::isModule('like') && isset($aFeed.feed_is_liked) && $aFeed.feed_is_liked} is_liked{/if}">
					{if Phpfox::isUser() && Phpfox::isModule('like') && isset($aFeed.like_type_id)}
					{if isset($aFeed.like_item_id)}
					<ul>{module name='like.link' like_type_id=$aFeed.like_type_id like_item_id=$aFeed.like_item_id like_is_liked=$aFeed.feed_is_liked}</ul>
					{else}
					<ul>{module name='like.link' like_type_id=$aFeed.like_type_id like_item_id=$aFeed.item_id like_is_liked=$aFeed.feed_is_liked}</ul>
					{/if}
					{if Phpfox::isUser()
			...
			<div class="{if isset($sFeedType) &&  $sFeedType == 'view'} feed_item_view{/if} comment_mini comment_mini_end">
				<form method="post" action="#" class="js_comment_feed_form">				
					...
					<div class="{if isset($sFeedType) &&  $sFeedType == 'view'}comment_mini_content {/if}comment_mini_textarea_holder">
						...
						<div class="js_feed_comment_process_form">{img theme='ajax/add.gif'}</div>
					</div>
