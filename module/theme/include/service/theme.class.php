...
{	
	...
	public function unity() {
		...
		foreach ($rows as $row) {
			...
			if ($row['folder'] == 'unity') {
				$row['name'] = 'Default';
				$row['image'] = 'http://d2h79mkp7etn4r.cloudfront.net/icon/2014/09/9c015e23637969877387b93dbce60888.jpg';
			} else {
				$data = $row['website'];
				if (substr($data, 0, 1) == '{') {
					$data = json_decode($data);
					$row['image'] = 'http://d2h79mkp7etn4r.cloudfront.net/icon/' . $data->i;
				}
			}
