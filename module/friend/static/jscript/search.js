	...
	{		
		...
		if ($iFound)
		{
			if (this._get('global_search')){
				$sHtml += '<li><a href="#" class="holder_notify_drop_link" onclick="$(\'#panel_content\').find(\'form:first\').submit(); return false;">' + oTranslations['friend.show_more_results_for_search_term'].replace('{search_term}',htmlspecialchars($oObj.value)) + '</a></li>';
			}
			if($('.js_temp_friend_search_form').length){
			...
			else if($('.panel_content').length){
				$('.panel_content').html('<ul class="search_results search_results_friends">' + $sHtml + '</ul>');
			}
		}
		else
		{
