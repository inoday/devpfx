...
{	
	...
	if (!bIsAlreadyOpen)
	{
		...
		if (bIsPhotoImage){
			...
			sHtml += '<a href="#" onclick="return js_box_remove(this);" class="js_box_close_it">' + oTranslations['core.close'] + '</a><span class="js_box_history">' + params[getParam('sGlobalTokenName') + '[call]'] + '</span>';
		...
		$.ajax(
			{
				...
				success: function(sMsg)
				{
					...
					if (bIsPhotoImage)
					{
						// $('#main_content_holder').hide();
						$('.js_box_image_holder_full_loader').remove();

						var thisHeight = $(window).height();
						var thisBodyHeight = $('body').height();

						if (!$('#photo_view_background').length) {
							$('body').prepend('<div id="photo_view_background"></div>');
						}
						$oNew.width('auto'); // .css('min-height', thisHeight);
						$('#photo_view_background').css('min-height', thisHeight);

						// $('#main_content_holder').css('opacity', '0.1');

						$('.js_box_close').hide();
						// $('.js_box_content, .photo_view_box_image').height(thisHeight);

						// p($('.js_box').innerWidth());
						$oNew.css({
							left: '50%',
							'margin-left': '-' + ($('.js_box').innerWidth() / 2) + 'px',
							top: ($(window).scrollTop() + 40)
						}).fadeIn();

						if ($oNew.find('.item_bar').length) {
							$('#photo_view_theater_mode').css('padding-bottom', $oNew.find('.item_bar').innerHeight());
						}
						...
						// $('.photo_view_box_comment').css('min-height', thisHeight);
						...
						{							
							...
							$('#photo_view_background').click(function()
							{			
								if (!bCanCloseImageBox)
								{
									bCanCloseImageBox = true;
								...
								else
								{
									...
									$oNew.fadeOut('fast', function() {$(this).remove();});
									// $('#main_content_holder').fadeIn();
...
$Behavior.thickboxBrowser = function(){
	...
	if ($Core.exists('.js_box_image_holder_full') && !$('#view_photo_click_image_holder').length){
